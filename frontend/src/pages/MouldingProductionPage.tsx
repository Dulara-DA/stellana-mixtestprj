...
import { CheckCircle2, CircleDot, Pencil, Play } from 'lucide-react'
...
export function MouldingProductionPage() {
  ...
  const [startForm, setStartForm] = useState({ pressId: '', cartId: '' })
  ...
  const usableCarts = carts.filter((cart) => ['RECEIVED_AT_MOULDING', 'PARTIALLY_CONSUMED'].includes(cart.status) && cart.remainingQuantity > 0)
  ...
  return (
    <div>
      <PageHeader
        ...
        description="One received blank produces one good tyre, one rejected tyre, or one rejected blank. Quantity validation is enforced again by the backend."
      ...
      {records.length === 0 ? <EmptyState title="No Moulding records" message="Receive a cart, then start the first press production record." /> : (
          ...
          <table>
            ...
            <tbody>{records.map((record) => (
              <tr key={record.id}>
                ...
                <td>
                  {canOperate && record.status === 'IN_PROGRESS' && <button className="btn-primary" onClick={() => { setCorrectionMode(false); setCompletion({ ...emptyCompletion, id: record.id, goodTyreQuantity: '' }) }}><CheckCircle2 size={16} /> Complete</button>}
                  ...
                  {!((canOperate && record.status === 'IN_PROGRESS') || (canCorrect && record.status === 'COMPLETED')) && <span className="text-xs text-slate-400">Recorded</span>}
                </td>
              </tr>
            ))}</tbody>
          </table>
