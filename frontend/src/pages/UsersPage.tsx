...
export function UsersPage() {
  ...
  const [form, setForm] = useState({ fullName: '', email: '', password: '', role: 'MIXING_OFFICER' as Role })
  ...
  const create = async (event: FormEvent) => {
    ...
    try {
      ...
      setShowCreate(false)
      setForm({ fullName: '', email: '', password: '', role: 'MIXING_OFFICER' })
      await load()
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-ink/60 p-8 backdrop-blur-sm">
          <form onSubmit={create} className="w-full max-w-xl rounded-2xl bg-white p-7 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <label><span className="label">Role</span><select className="field" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as Role })}>{['MANAGER', 'MIXING_OFFICER', 'SYSTEM_ADMIN', 'STORES_OFFICER', 'LAB_OFFICER'].map((role) => <option key={role} value={role}>{humanize(role)}</option>)}</select></label>
            </div>
